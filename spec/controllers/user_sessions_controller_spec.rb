...
describe UserSessionsController do
  ...
  describe "POST 'create'" do
    context "with correct credentials" do
      ...
      it "sets the user id to the session" do
        post :create, email: "jason@teamtreehouse.com", password: "treehouse1"
        expect(session[:user_id]).to eq(user.id)
      end

      it "sets the flash message for successful log in" do
        post :create, email: "jason@teamtreehouse.com", password: "treehouse1"
    ...
    shared_examples_for "denied login" do
      ...
      it "sets the flash error message" do
        post :create, email: email, password: password
        expect(flash[:error]).to eq("Invalid log in. Please check your email and password..")
      end
    end
